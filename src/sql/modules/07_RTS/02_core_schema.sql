...
-- 6. [CORE].[UserMaster] DeptId Upgrade
IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = 'CORE' AND TABLE_NAME = 'UserMaster' AND COLUMN_NAME = 'DeptId')
BEGIN
    ALTER TABLE [CORE].[UserMaster] ADD [DeptId] INT NULL;
END;
GO
